...
import rospy, cv2, numpy, math, pickle, result
...
if __name__ == "__main__":
    ...
    formated_plant_frames = {}
    for position, plant_frames in filtered_center_dictionary.items():
        if position[4] > 0:
            for index, plant_frame in enumerate(plant_frames):
                current_position = list(position[:-1])
                current_position.append(index)
                current_position = tuple(current_position)
                if current_position not in formated_plant_frames.items():
                    formated_plant_frames[current_position] = []
                formated_plant_frames[current_position].append(plant_frame)
        else:
            for index in range(len(plant_frames)-1, -1, -1):
                current_position = list(position[:-1])
                current_position.append(len(plant_frames)-1-index)
                current_position = tuple(current_position)
                if current_position not in formated_plant_frames.items():
                    formated_plant_frames[current_position] = []
                formated_plant_frames[current_position].append(plant_frames[index])
    print(formated_plant_frames)
    '''
    publishers = {}
