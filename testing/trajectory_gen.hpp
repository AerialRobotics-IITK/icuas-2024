...
class trajectory_gen{
	...
	public:
		...
		void create_graph(double x, double y,double z){
			...
			auto temp_node =  std::make_shared<node>(std::shared_ptr<node> (new struct node));
			temp_node->x = 0; temp_node->y = 0; temp_node->z = 0;
			nodes.push_back(temp_node);

			temp_node =  std::make_shared<node>(std::shared_ptr<node> (new struct node));
		...
		trajectory_gen(double x, double y, double z, double x_offset, double y_offset, double z_offset, std::vector<int> Points){
			...
			std::cout<<std::endl<<std::endl<< "Total no of points with plant:" <<final_path.size();
		...
		void get_final_waypoints(){
			for(int i=0;i<final_waypoints;i++){
				final_waypoints[i][0] = final_waypoints[i][0] + x_offset;
				final_waypoints[i][1] = final_waypoints[i][1] + y_offset;
			    final_waypoints[i][2] = final_waypoints[i][2] + z_offset;
			}
			return final_waypoints;
		}
